    ...
    401 "Token expired"     檢查撤銷狀態
        ...
        401 "Token revoked"       檢查權限範圍
                              ...
                              權限不足？         有權限
                                   │               │
                                   ↓               ↓
        403 "Insufficient permissions"    記錄 Audit Log
                                                   │
                                                   ↓
                                              允許存取
```
...
│   ├── test_redis_cache.py     # Redis 快取測試（5 個測試）
│   └── README.md               # 測試說明文件
│
└── data/                       # 資料檔案
    ├── uploads/                # 使用者上傳的檔案
...
### examples.sh - API 使用示範

完整演示系統的 API 使用流程，相當於可執行的文檔。
...
1. 📝 註冊新用戶
2. 🔐 登入取得 JWT Token
3. 🎫 使用 JWT 創建 Personal Access Token（fcs:analyze）
4. 📋 列出所有 Token
5. 📊 使用 PAT 存取 FCS 資料：
   - 取得參數資訊
   - 取得事件資料
   - 取得統計分析
6. ✅ 測試權限階層繼承（fcs:analyze 包含 fcs:read）
7. ❌ 測試跨資源權限限制（fcs:analyze 不包含 workspaces:read）
8. 📜 查看 Token 使用日誌
9. 🎫 創建更多不同權限的 Token（workspaces:admin）
10. 🔒 測試權限驗證（admin → read/write/delete）
11. 🗑️ 撤銷 Token
12. ❌ 測試撤銷後的 Token（應該失敗）

**輸出範例**：
```bash
=== PAT Auth System API Examples ===
...
3. Creating a Personal Access Token with fcs:analyze permission...
PAT Token: pat_a1b2c3d4e5f6...

5. Using PAT to get FCS parameters...
{
  "success": true,
  "data": {
    "total_events": 34297,
...
系統提供三種驗證方式，涵蓋不同的使用場景。

### 1. 自動化測試（pytest）

完整的單元測試和整合測試，用於開發和 CI/CD。
...
**測試套件**（共 35+ 個測試案例）：
- `test_permissions.py` - 權限階層繼承測試（4 個測試）
- `test_token_expiry.py` - Token 過期和撤銷測試（5 個測試）
- `test_token_storage.py` - Token 安全儲存測試（5 個測試）
- `test_token_regenerate.py` - Token 重新產生測試（7 個測試）
- `test_token_ip_whitelist.py` - Token IP 白名單測試（9 個測試）
- `test_redis_cache.py` - Redis 快取測試（5 個測試）
...
| 項目 | pytest | verify.sh | examples.sh |
|------|--------|-----------|-------------|
| **目的** | 代碼邏輯測試 | 系統健康檢查 | API 使用演示 |
| **深度** | 深（測試邊界條件） | 淺（基本可用性） | 中（完整流程） |
| **執行時機** | 開發時/提交前 | 部署後 | 任何時候 |
| **測試數據** | Fixtures | 真實請求 | 真實請求 |
| **測試對象** | 函數/類/模組 | 整個系統 | API 端點 |
| **失敗影響** | 代碼有 bug | 系統有問題 | 演示失敗 |
...
### 問題 5: examples.sh 執行時用戶已存在

```bash
# examples.sh 會自動處理已存在的用戶
# 如果需要完全重置，可以刪除數據庫並重新啟動：
docker-compose down -v
docker-compose up -d
sleep 15
./examples.sh
